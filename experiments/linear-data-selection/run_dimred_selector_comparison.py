...
from data_selection_sims.experiment_modules.dimred_selector_comparison import (
...
def _parse_regimes(value: str | None, default: tuple[str, ...]) -> tuple[str, ...]:
    if value is None:
        return default
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the high-dimensional selector-comparison experiment.")
    ...
    parser.add_argument("--r", type=int, default=20, help="True reference rank.")
    ...
    parser.add_argument("--seeds", type=int, default=5, help="Number of random seeds / repeats.")
    parser.add_argument("--score-rank-pairs", default="5:5,10:10,20:20", help="Comma-separated K_R:K_T pairs.")
    parser.add_argument("--spike-high", type=float, default=25.0, help="Largest spike in the reference spectrum.")
    parser.add_argument("--spike-low", type=float, default=2.0, help="Smallest spike among the retained reference directions.")
    parser.add_argument("--tail-eig", type=float, default=0.05, help="Flat tail eigenvalue outside the reference rank.")
    parser.add_argument(
        "--spectrum-decay",
        choices=["geometric", "linear", "powerlaw"],
        default="geometric",
        help="Decay profile for the reference spikes.",
    )
    parser.add_argument("--powerlaw-exponent", type=float, default=1.0, help="Exponent used when --spectrum-decay=powerlaw.")
    ...
    parser.add_argument(
        "--target-spectrum-decay",
        choices=["isotropic", "geometric", "linear", "powerlaw"],
        default="isotropic",
        help="Decay profile for the target covariance. Use isotropic to keep Sigma_T = scale * I.",
    )
    parser.add_argument(
        "--target-powerlaw-exponent",
        type=float,
        default=None,
        help="Optional exponent used when --target-spectrum-decay=powerlaw. Defaults to --powerlaw-exponent.",
    )
...
def main() -> None:
    ...
    config = DimRedSelectorConfig(
        ...
        spectrum_decay=str(args.spectrum_decay),
        powerlaw_exponent=float(args.powerlaw_exponent),
        ...
        target_spectrum_decay=None if args.target_spectrum_decay == "isotropic" else str(args.target_spectrum_decay),
        target_powerlaw_exponent=None if args.target_powerlaw_exponent is None else float(args.target_powerlaw_exponent),
        epsilon=float(args.epsilon),
        alpha=float(args.alpha),
        rho=None if args.rho is None else float(args.rho),
        regimes=_parse_regimes(args.regimes, default_config.regimes),
    )
