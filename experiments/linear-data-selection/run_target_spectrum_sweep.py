...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a repeated sweep over target covariance spectra.")
    ...
    parser.add_argument("--seeds", type=int, default=10, help="Number of random seeds / repeats per target-spectrum setting.")
    parser.add_argument(
        "--score-rank-pairs",
    ...
    parser.add_argument(
        ...
        default="geometric",
        help="Decay profile for the reference Fisher spikes.",
    )
    ...
    parser.add_argument(
        ...
        help="Comma-separated target decay families to sweep.",
    )
    parser.add_argument(
        "--target-powerlaw-exponents",
        default="0.5,1.0,1.5,2.0",
        help="Comma-separated target powerlaw exponents to sweep when target decay is powerlaw.",
    )
    parser.add_argument(
        "--comparison-powerlaw-exponent",
        type=float,
        default=1.0,
        help="Which target powerlaw exponent to use in the decay-family facet.",
    )
...
def main() -> None:
    ...
    dimred_config = DimRedSelectorConfig(
        ...
        target_powerlaw_exponent=float(args.comparison_powerlaw_exponent),
        epsilon=float(args.epsilon),
        alpha=float(args.alpha),
        rho=None if args.rho is None else float(args.rho),
        regimes=_parse_string_list(args.regimes),
    )
    sweep_config = TargetSpectrumSweepConfig(
        ...
        comparison_powerlaw_exponent=float(args.comparison_powerlaw_exponent),
    )
    manifest = run_target_spectrum_sweep(
